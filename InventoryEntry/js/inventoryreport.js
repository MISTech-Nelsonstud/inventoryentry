...
INVREPORT.initGrid = function (company, mess) {
    var isComplete = true;
    var params = [
        company
    ]
    ...
    keys.forEach(function (item) {
        if (INVREPORT.reportData.rows[item][12] === "No") {
            isComplete = false;
        }
    });
    ...
    var gridIndex = DISPLAYGRID.addGrid(INVREPORT.displayDivId, "divInvReportGrid", null, null, 20);
    DISPLAYGRID.addColorDefinition(gridIndex, 2, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addColorDefinition(gridIndex, 3, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addColorDefinition(gridIndex, 4, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addColorDefinition(gridIndex, 5, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addColorDefinition(gridIndex, 6, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addColorDefinition(gridIndex, 12, 12, function (val) {
        return val === "No" ? "#ff9999" : "#80ff80"
    })
    DISPLAYGRID.addTitles(gridIndex, company + " Report <input id=\"butExcel\" type=\"button\" value=\"Create Excel\" onclick=\"INVREPORT.createExcel('" + company + " Report');\"></input><input id=\"butCsv\" type=\"button\" value=\"Create CSV\" onclick=\"INVREPORT.createCsv('" + company + " Report');\"></input><input id=\"butUpdate\" type=\"button\" value=\"Update Inventory\" onclick=\"INVREPORT.updateConfirm('" + company + "');\"></input><div>", (isComplete ? "<span style=\"color:green\">All inventory for " + company + " is complete</span>" : "<span style=\"color:red\">All inventory must be complete for " + company + " before you can create a report and update inventory</span>"));
...
INVREPORT.createExcel = function (title) {
    var numbercolumns = [];
    for (i = 0; i < INVREPORT.reportData.columns.length; i += 1) {
...
INVREPORT.createCsv = function (title) {
    var csvColumns;
    var csvRows = [];
    newcolumns = [];
    newrows = [];
    ...
    for (var i = 0, l = INVREPORT.reportData.rows.length; i < l; ++i) {
        newrows.push([INVREPORT.reportData.rows[i][0], INVREPORT.reportData.rows[i][1].substring(0,4), INVREPORT.reportData.rows[i][2], INVREPORT.reportData.rows[i][3], INVREPORT.reportData.rows[i][5], INVREPORT.reportData.rows[i][6], INVREPORT.reportData.rows[i][9]]);
    ...
    if (window.navigator.msSaveOrOpenBlob) {
        var blob = new Blob([decodeURIComponent(encodeURIComponent(csvString))], {
...
INVREPORT.updateInventory = function (result, company) {
    ...
    if (result) {
        ...
        if (isComplete) {
            //do the update
            var update = AJAXPOST.callQuery2("WMS_Update_PhysInvToCurrentInv", params);
