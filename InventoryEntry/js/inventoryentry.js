...
INVE.Job = function (row, hasLXLocations, allLocs, jobId) {
    "use strict";
    this.name = row[0];
    this.usewmslocations = (row[1] === "1");
    this.uselxlocations = hasLXLocations;
    this.wmssite = row[2];
...
INVE.utility = {
    ...
    getJobInfo: function () {
        ...
        INVE.currentJob = new INVE.Job(row, hasLXLocations, allLocs, jobId);
    },
    title: function (title) {
        "use strict";
        var obj = document.getElementById("divTitle");
        obj.innerHTML = "<div class=\"inventoryEntryLeftTitle\">Inventory" + ((title && title !== "") ? " - " + title : "") + "</div>" +
...
INVE.page1 = {
    ...
    display: function (mess) {
        ...
        iHTML = "<button onclick=\"INVE.page1.buttonFilter(-1);\" style=\"color:white; background-color:#202020; margin-bottom:2px;padding:0 2px;\">Show All</button><button onclick=\"INVE.page1.buttonFilter(0);\" style=\"margin-bottom:2px;padding:0 2px;\">Not Finished</button>" +
    ...
    filter: function (pageNumber, init, val) {
        ...
        var paginate = function () {
            ...
            INVE.currentJob.locs.forEach(function (item, index) {
                ...
                var filtertest = (exp.test(item.loc.toUpperCase()) && (INVE.userPreference.descriptionfilter === "" || item.locdescript + (item.iswms ? " (WMS)" : " (LX)") === INVE.userPreference.descriptionfilter));
        ...
        if (INVE.page1.pages.length > 1) {
            ...
            INVE.page1.pages.forEach(function () {
                var thisPageNumber = arguments[1] + 1;
        ...
        selectedPage.forEach(function (itemindex, index) {
            ...
            if (exp.test(item.loc.toUpperCase()) && (INVE.userPreference.descriptionfilter === "" || item.locdescript + (item.iswms ? " (WMS)" : " (LX)") === INVE.userPreference.descriptionfilter)) {
                iHTML += "<button id=\"butLocSelect" + String(index) + "\"" + btnStyle + " data-loc=\"" + String(index) + "\" onclick=\"INVE.page2.display(" + String(index) + ");\">" + 
...
INVE.page2 = {
    ...
    display: function (locIndex, mess) {
        ...
        INVE.utility.title("Loc: " + selectedLocation.loc + (selectedLocation.iswms ? " (WMS)" : " (LX)"));
        var infoObj = {};
        if (mess && mess !== "") {
        ...
        var dispObj = document.getElementById(INVE.displayDivId);
        var topObj = COMMON.getBasicElement("div", "divAddItem", COMMON.getHTMLButton(null, "Add Item Not Listed", "INVE.page2.newItem(0);"));
        topObj.appendChild(COMMON.getHTMLButton(null, "Return to \"Select Location\"", "INVE.page1.display(" + String(locIndex) + ");"));
        ...
        res.payload.rows.forEach(function (row, index) {
            ...
            var title = "<div><h3>" + thisItem.itemnumber + "</h3><div>Lot: " + thisItem.lot + "</div><div>" + thisItem.description + "</div></div>";
            var buttonIHTML = "<button" + (thisItem.hasrecord ? " style=\"background-color:#646F45\"" : "") + " onclick=\"INVE.page3.init(" + String(locIndex) + ", " + String(index) + ");\" class=\"inventoryEntryPage2ItemAdded\" >" + title + "</button>";
        ...
        iHTMLbuttons = iHTMLaddedItems + iHTMLbuttons + "<div style=\"clear:both\"></div>";
    ...
    printitemtag: function(invid){
        ...
        if (printZebra) {
            ...
            var res = AJAXPOST.callQuery2("WMS_PhysInv_CreatePlainPaperTag", [String(invid), printerId]).payload.rows[0];
            var params = [];
    ...
    newItemValidate: function(dataValues, formIndex){
        ...
        if (res.payload.rows[0][0] === "1") {
            FILLIN.errorMessage(formIndex, "This Item number is incorrect, please enter the corrent item number");
            return false;
        }
        return true;
    },
    newItemActions: function (dialogResults, dataValues) {
        ...
        INVE.page2.newItemObj = new INVE.InventoryItemObject(dataValues.txtItem.value);
        var res = AJAXPOST.callQuery2("WMS_PhysInv_GetItemDescription", [dataValues.txtItem.value, INVE.currentJob.jobid]);
    ...
    newItemLot: function (lot, formIndex) {
        ...
        INVE.page3.init(locIndex, itemIndex);
    }
};
INVE.page3 = {
    formindex: null,
    selectedLocation: null,
    selectedItem: null,
    init: function(locIndex, itemIndex){
        "use strict";
        window.scrollTo(0, 0);
    ...
    display: function (actioncode) {
        ...
        var formIndex = FILLIN.createForm("divTxtQty", "Loc: " + selectedLoc.loc + " - " + selectedItem.itemnumber, "Enter Count. If you are counting countainers enter Quantity per container or leave blank or zero for item count", INVE.page3.displayActions, null, "97%");
        ...
        FILLIN.addTextBox(formIndex, "txtQty", null, "Count", true, "integer", null, null, true, null, null, null, "butContinue", { "style": "width: 205px; font-size: 1.6em; height:32px;" });
        FILLIN.addFreeButton(formIndex, "Continue", "butContinue", "Continue", true, false);
        FILLIN.addTextBox(formIndex, "txtContainer", "0", "Pieces Per Container (Zero or Blank if counting individual pieces)", false, "integer", null, null, true, null, null, null, "butContinue", { "onkeypress": "INVE.page3.currentCountObj();", "style": "width: 95%; font-size: 1.6em; height:32px;" });
        FILLIN.addGenericControl(formIndex, COMMON.getBasicElement("div", "divTot"), "", true);
        FILLIN.addButton(formIndex, "Return", "butReturn", "Return", false, false, true);
        FILLIN.displayForm(formIndex);
    ...
    //todo - change the plain paper print out 1/2 page, etc.s remove zebra labels, change the buttons on the count screen so they don't go under buttons, move total count to banner
    additionalCounts: function () {
        ...
        FILLIN.addFreeButton(formIndex, true, null, "More Counts");
        ...
        INVE.page3.countsentered.forEach(function (countObj) {
            if (countObj.count !== null) {
                ...
                if (countObj.containerpieces === 0) {
                    mess += "<tr><td style=\"font-weight:600;color:red;text-align:right;padding:0 3px;\">" + String(countObj.count) + "</td><td style=\"font-weight:700;color:red;text-align:left;padding:0 3px;\">pieces</td></tr>";
                    tot += countObj.count;
                } else {
                    mess += "<tr><td style=\"font-weight:600;color:red;text-align:right;padding:0 3px;\">" + String(countObj.count * countObj.containerpieces) + "</td><td style=\"font-weight:700;color:red;text-align:left;padding:0 3px;\">pieces (" + String(countObj.count) + " containers of " + String(countObj.containerpieces) + ")</td></tr>";
